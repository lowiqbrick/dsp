...
pub mod items {
    ...
    use super::essentials::{item_logic::ItemAmount, AssemblerMK, ChemLabMK, LabMK, ProgamInfo, Proliferator, SmelterMK, ArgState};
        ...
        while keep_looping {
            ...
            if index == 0 {
                if item_hashmap.contains_key(&args[index]) {
                    ...
                    settings.produced_item =
                        ItemAmount::new(
                            args[1].parse::<u8>().expect("provided invalid number"),
                ...
                // don't look at the rest of this loop
                continue;
            }
            // arguments that are boolean
            // squash the output
            if args[index] == "-s" || args[index] == "--squash" {
                settings.squash = true;
            }
            // assume basics
            ...
            if args[index].contains("proliferator") {
                match args[index].as_str() {
                    ...
                    _ => eprintln!("the argument {} isn't valid and won't be incorporated", args[index]),
                }
            }
            ...
            if args[index].contains("furnace") {
                match args[index].as_str() {
                    "furnace=1" => settings.smelter = SmelterMK::ArcSmelter,
                    "furnace=2" => settings.smelter = SmelterMK::PlaneSmelter,
                    "furnace=3" => settings.smelter = SmelterMK::NegentropySmelter,
                    _ => eprintln!("the argument {} isn't valid and won't be incorporated", args[index]),
                }
            }
            ...
            if args[index] == "-f" || args[index] == "--furnace"{
            ...
            if status == ArgState::AdditionalItems {
                if index >= args_len + 2  {
                    if item_hashmap.contains_key(&args[index]) {
                        settings.additional_items.push(
                            ItemAmount::new(args[index+1].parse::<u8>().expect("provided invalid number"),
                            args[index].clone()));
                    } else {
                        eprintln!("{} isn't a valid item", args[index]);
                    }
            ...
            if status == ArgState::ItemRecipe {
                if index +1 <= args_len -1 {
                    if item_hashmap.contains_key(&args[index]) {
                        ...
                        let recipe_num: u8 = args[index+1].parse::<u8>().expect("provided invalid number");
                        ...
                        match item_rec_num_opt {
                            Some(t) => {
                                ...
                                if recipe_tot_num - 1 >= recipe_num as usize {
                                    settings.item_recipe.push(
                                        ItemAmount::new(recipe_num,
                                        args[index].clone()));
                                }
                            },
